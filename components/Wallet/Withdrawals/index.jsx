...
const Withdrawals = ({ data }) => {
  ...
  useEffect(() => {
    if(user){
      const showList = async () => {
        const res = await get(query(ref(db, `withdrawals/${user.uid}`), limitToLast(30), orderByChild("createdAt")));
        if (res.hasChildren()) {
          setListWithdraws([...Object.values(res.val())
            .map((val) => {
  ...
  const filterWithdraw = (fil) => {
    const newWithdraw = listWithdraws.filter(withdraw => {
  ...
  const sort = (e) => {
    if (sorting === e.target.id) {
      const newList = [...listWithdraws].reverse()
      setListWithdraws(newList)
    } else {
      setSorting(e.target.id)
      const sortWithdraws = listWithdraws.sort((a, b) => {
      ...
      setListWithdraws(sortWithdraws)
    }
  }
                ...
                return (
                  <tr key={index} className='h-12'>
                    ...
                    <td
                      className='border border-slate-100'
                    >
                      <p>{withdraw.amount.toLocaleString('es-ar', { style: 'currency', currency: withdraw.currency, minimumFractionDigits: 2 })}</p>
                    </td>
                    <td
                      className='border border-slate-100'
                    >
                      <p>{withdraw.status ? "Transferido" : "Solicitado"}</p>
                    </td>
                  </tr>
                );
              })
              :
              listWithdraws?.map((withdraw, index) => {
                return (
                  <tr key={index} className='h-12'>
                    <td
                      className='border border-slate-100'
